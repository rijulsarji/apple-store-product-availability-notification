...
function buildUrl() {
  ...
  const parts = [`parts.0=${encodeURIComponent(PART)}`, "fae=true", "little=false",
                 "mts.0=regular", "mts.1=sticky", "fts=true", "searchNearby=true"];
...
(async () => {
    ...
    while (true) {
        try {
            ...
            const data = await page.evaluate((url, referer, userAgent) => {
                return fetch(url, {
                    headers: {
            ...
            if (data.error) {
                throw new Error(`Browser fetch failed with status ${data.status} ${data.statusText}`);
            }
            ...
            if (stores.length > 0) {
                const availableStores = stores.filter(store => store.partsAvailability[PART]?.pickupDisplay !== "unavailable");
            ...
            try {
                ...
                console.error("Fatal: Could not recover session after error. Exiting.", seedError.message);
                await browser.close();
                process.exit(1);
            }
        }
